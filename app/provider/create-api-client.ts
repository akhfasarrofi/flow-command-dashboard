...
import { ApiInstance, buildURL } from './http';
...
type FetchResult<T> = FetchState<T> & {
...
export default function createApiClient(options: HttpInstanceOptions = {}) {
  ...
  function useFetch<T>(
    url: string,
    config?: Omit<HttpConfig, 'onUpload' | 'onDownload'>,
    ...
    const fetchData = useCallback(
      async (refetch: boolean = false): Promise<FetchState<T>> => {
        if (!enabled && !refetch) {
          return { cacheKey: null, data: null, error: null, isLoading: false };
        }
  ...
  function useMutation<TData, TRequest = void>(
    ...
    const mutate = useCallback(
      async (request?: TRequest): Promise<ApiResponse<TData>> => {
        if (abortControllerRef.current) abortControllerRef.current.abort();
        abortControllerRef.current = new AbortController();
        ...
        try {
          ...
          requestKey = JSON.stringify({ method, request, url });
          ...
          if (method === 'GET') {
            ...
            requestPromise = instance.request<TData>({
              ...
              onUpload: shouldTrackUpload
                ...
                : undefined,
              signal: abortControllerRef.current.signal,
              url: config?.queryMutation ? buildURL(url, request as HttpConfig['params']) : url,
            });
          }
          ...
          setState({ cacheKey: null, isLoading: false, progress: null });
          throw err;
        } finally {
